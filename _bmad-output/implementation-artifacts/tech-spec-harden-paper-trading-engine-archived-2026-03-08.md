...
- [ ] Task 2: Create Market Analytics Module (Data API Client)
  ...
  - Action: Create new module with the following functions:
    ...
    def estimate_slippage_from_trades(
        ...
        total_volume = sum(t["size"] * t["price"] for t in asset_trades)
        total_shares = sum(t["size"] for t in asset_trades)
        ...
        # Adjust for side (buying pushes price up, selling pushes down)
        if side == "BUY":
        ...
        estimated_price = max(0.01, min(0.99, estimated_price))
...
- [ ] Task 4: Add Edge Calibration and Stale Data Detection
  ...
  - Action: Modify `compute_edge()` function (lines 256-361):
    ...
    2. After line 324 (`edge = model_prob - live_yes`), add calibration:
       ```python
       # Edge calibration for hardened mode
       if hardened:
           from nba_bot.config import EDGE_DECAY_FACTOR, EDGE_CONFIDENCE_THRESHOLD
           edge = edge * EDGE_DECAY_FACTOR
           edge = max(min(edge, EDGE_CONFIDENCE_THRESHOLD), -EDGE_CONFIDENCE_THRESHOLD)
       ```
...
- [ ] Task 5: Add Fee Deduction to Settlement
  ...
  - Notes: Fees are deducted at settlement, not execution, matching Polymarket behavior
...
- [ ] Task 6: Add Hardened Paper Trade Execution
  ...
  - Action: Add new functions and modify `execute_paper_trade()`:
    ...
    2. Add game exposure tracking function after `has_active_position()`:
       ...
       def get_game_exposure(event_slug: str) -> float:
           ...
           for trade in load_trades():
               if trade.get("event_slug") == event_slug and trade.get("status") == "PENDING":
    ...
    3. Add fill probability function:
       ...
       ```
    4. Add slippage application function:
       ```python
       def apply_slippage(
           enter_price: float,
           direction: str,
           ...
           Uses Data API if available, falls back to model.
           ...
           if asset_id:
               ...
               adj_price, slip = estimate_slippage_from_trades(
                   asset_id, shares, "BUY" if "YES" in direction else "SELL"
               )
               if adj_price is not None:
                   return adj_price, slip

           # Fallback: model-based slippage
           ...
           return min(max(adjusted, 0.01), 0.99), slippage
       ```
    5. Add latency simulation function:
       ```python
    ...
    6. Create new `execute_paper_trade_hardened()` function:
       ```python
       def execute_paper_trade_hardened(alert: dict) -> bool:
           """
           ...
           # Fail-fast: Data API required for hardened mode
           if not check_data_api_available():
               logger.error("Data API unavailable — cannot execute hardened trade")
               raise RuntimeError("Data API required for hardened paper trading")
...
- [ ] AC 4: Given a winning hardened trade with $50 payout, when settlement runs, then a $1 fee (2%) is deducted and the trade records `fee: 1.00`
...
- [ ] AC 7: Given `--seed 42` is passed, when multiple hardened trades execute, then the stochastic outcomes (fill/drift) are reproducible across runs
